...
const LOGS_DIR = process.env.BUILDS_DIR || './logs';
...
function get_log_file(commit) {
    return `${LOGS_DIR}/${new Date().getTime()}-${commit}.log`;
}
...
function set_status_pending_and_enqueue(github_conf, docker_conf) {
    ...
    const log_file = get_log_file(github_conf.commit);
    ...
    ghrepo.status(github_conf.commit, {
      "state": "pending",
      "target_url": `${BUILD_LOGS_BASE_URL}/${log_file}`,
...
module.exports = function BuildQueue(redis_url) {
    ...
    builds_queue.on('completed', function(job, result){
        ...
        ghrepo.status(job.data.github.commit, {
          "state": "success",
          "target_url": `${BUILD_LOGS_BASE_URL}/${job.data.log_file}`,
