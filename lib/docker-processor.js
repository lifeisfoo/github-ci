...
function calc_elasped_secs_from(start_date) {
...
module.exports = function(job, done){
    ...
    clone_build_test_cmd.on('close', (exit_code) => {
        log_stream.write(`Build duration: ${calc_elasped_secs_from(start_date)} seconds\n`);
    ...
    clone_build_test_cmd.on('error', (err) => {
        //log_stream.write(`Build duration before error: ${calc_elasped_secs_from(start_date)} seconds\n`);
        done(err);
    });
}